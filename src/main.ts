...
const board = new Board(sideLengthInCells)
let rules = {
	rule1: {
		liveDead: 1,
		numNeighbors: 2,
...
const changeBoardSize = (size: number) => {
	...
	board.grid = Board.makeBoardArrayNumber(sideLengthInCells)
	canvasColors = Board.makeBoardArrayString(sideLengthInCells)
	populate()
...
const checkColor = (row: number, col: number, div: HTMLDivElement) => (div.style.backgroundColor = board.grid[row][col] ? Board.colors.draw : Board.colors.light)
...
window.addEventListener(
	...
	() => {
		canvasColors = Board.makeBoardArrayString(sideLengthInCells)
		Canvas.ctx.canvas.height = editBoard.clientHeight
		Canvas.ctx.canvas.width = editBoard.clientWidth
		setSliderInfoBubble(rawSlider, rangeInfoBubble)
		populate()
	},
	{ passive: true }
...
window.addEventListener(
	...
	() => {
		Canvas.ctx.canvas.width = editBoard.clientWidth
		Canvas.ctx.canvas.height = editBoard.clientHeight
		if (!isEditing) Canvas.renderCanvas()
	},
	{ passive: true }
)
...
toggleBWBtn.addEventListener(
	...
	() => {
		...
		Board.colors.draw = isBlackWhite ? '#000000' : colorPicker.value
		let count = 0
		for (let row = 0; row < board.grid.length; row++) {
			for (let col = 0; col < board.grid[0].length; col++) {
				const div = document.getElementById(String(count)) as HTMLDivElement
